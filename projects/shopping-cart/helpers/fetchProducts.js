const fetchProducts = async (QUERY) => { // função asincrona recebendo o parâmetro para complementar o endpoint
  // seu código aqui
  if (!QUERY) throw new Error('You must provide an url'); // Retorna um erro caso não QUERY não seja um elemento válido
  const endpoint = `https://api.mercadolibre.com/sites/MLB/search?q=${QUERY}`; // complementa o endpoint para chamar a API com fetch abaixo
